	...
	([ox oy oz dist count]
		...
		(dotimes [n count]
			(let [
				...
				(def xvals (conj xvals (* (* (CMath/Sin yaw) (CMath/Cos pitch)) randdist)))
				(def yvals (conj yvals (* (CMath/Sin pitch) randdist)))
				(def zvals (conj zvals (* (* (CMath/Cos yaw) (CMath/Cos pitch)) randdist)))
			)
		)
		(make-data-points-3d xvals yvals zvals)
	)
	...
	([ox oy oz dist height count]
		...
		(dotimes [n count]
			(let [
				...
				(def xvals (conj xvals (* (CMath/Sin yaw) (rand dist))))
				(def yvals (conj yvals (- randheight (/ height 2))))
				(def zvals (conj zvals (* (CMath/Cos yaw) (rand dist))))
			)
		)
		(make-data-points-3d xvals yvals zvals)
	)
